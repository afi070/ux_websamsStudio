...
<body class="text-slate-800">
        ...
        <div class="flex space-x-6 items-center">
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-cloak
                    ...
                    <a href="#" class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition"><svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>Data Diri</a>
                    <a href="#" class="flex items-center justify-between px-4 py-2.5 hover:bg-slate-50 transition"><div class="flex items-center gap-3"><svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>0 Koin</div><svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></a>
                    <a href="#" class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition"><svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2zM10 8.5a.5.5 0 11-1 0 .5.5 0 011 0z"></path></svg>Isi Saldo</a>
                    <a href="#" class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition"><svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>Riwayat Top-up</a>
                    <a href="#" class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition"><svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"></path></svg>Riwayat Ticket</a>
                    <a href="{{ route('film.watchlist') }}" class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition"><svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>Tambahkan ke Watchlist</a>
    ...
    <div x-data="{
        activeSlide: 0,
        slides: [
            { title1: 'BUY 1', title2: 'GET 1', label: 'FREE!', period: '04 - 06 JUN 2026', desc: 'Promo ini hanya berlaku di Sams Studios Pulau Jawa <br><span class=\'text-red-600\'>kecuali Labuan Bajo</span>', img: 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?q=80&w=600&auto=format&fit=crop', movie: 'NOBODY LOVES KAY', creator: 'Sebuah Film Karya Bernardus Raka' },
            { title1: 'SPECIAL', title2: 'DISC', label: '50%', period: '07 - 10 JUN 2026', desc: 'Diskon spesial untuk member baru di semua cabang Sams Studios!', img: 'https://images.unsplash.com/photo-1574375927938-d5a98e8ffe85?q=80&w=600&auto=format&fit=crop', movie: 'THE MYSTERY', creator: 'Sebuah Film Karya Sutradara X' }
        ]
    }" x-init="setInterval(() => { activeSlide = (activeSlide + 1) % slides.length }, 5000)" 
    class="bg-[#f0dfc8] relative h-[340px] w-full overflow-hidden grid grid-cols-1 grid-rows-1">
        ...
        <template x-for="(slide, index) in slides" :key="index">
            <div x-show="activeSlide === index" 
                 ...
                 class="col-start-1 row-start-1 w-full h-full px-8 py-12 flex flex-col md:flex-row justify-between items-center">
                ...
                <div class="flex flex-col items-start z-10 space-y-3 mb-6 md:mb-0">
                    <div class="bg-[#003870] text-white p-6 rounded-br-[40px] rounded-tl-[20px] shadow-lg flex flex-col items-center justify-center transform -rotate-2">
                        <span class="text-3xl font-extrabold tracking-tight" x-text="slide.title1"></span>
                        <span class="text-4xl font-black text-yellow-400 my-1 tracking-wide border-y-2 border-yellow-400 px-2" x-text="slide.title2"></span>
                        <span class="text-2xl font-bold bg-cyan-400 text-[#003870] px-4 py-0.5 mt-2 rounded font-mono tracking-widest" x-text="slide.label"></span>
                    </div>
                ...
                <div class="text-center z-10 flex flex-col items-center space-y-4 max-w-md">
                    <div>
                        <p class="text-[#004785] font-black text-sm tracking-widest mb-1">PERIODE</p>
                        <div class="bg-cyan-500 text-white font-bold px-6 py-1.5 rounded-full shadow-inner text-sm tracking-wide" x-text="slide.period"></div>
                    </div>
                    <p class="text-[#004785] font-bold text-xs uppercase tracking-tight max-w-xs leading-relaxed" x-html="slide.desc"></p>
                </div>

                <div class="relative w-full md:w-[450px] h-[220px] bg-cover bg-center rounded-lg shadow-md overflow-hidden" :style="'background-image: url(' + slide.img + ');'">
                    <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-4 text-white">
                        <div class="bg-yellow-400 text-black font-black text-xs px-2 py-1 rounded w-max mb-1" x-text="slide.movie"></div>
                        <p class="text-[10px] opacity-80 uppercase tracking-widest" x-text="slide.creator"></p>
                    </div>
                </div>
            </div>
        </template>
        ...
        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 z-20">
            <template x-for="(slide, index) in slides" :key="index">
                <button @click="activeSlide = index" class="w-2 h-2 rounded-full transition-all duration-300" :class="activeSlide === index ? 'bg-[#003870] w-6' : 'bg-white'"></button>
            </template>
        </div>
    </div>
